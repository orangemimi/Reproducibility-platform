  ...
  methods: {
    ...
    initInstance() {
      ...
      getExpectedInstances.forEach((model) => {
        ...
        if (service.behavior.parameters != undefined) {
          x = 1;
   ...
   <Configuration>`;
      getExpectedInstances.forEach((model) => {
        ...
        service.behavior.inputs.forEach((input) => {
          this.procedureContent += `<Input name="${input.name}" isOptional="${input.isOptional}" dataServiceId="${input.dataServiceId}"/>`;
        });

        this.procedureContent += `</Inputs>`;
        if (service.behavior.parameters != undefined) {
        ...
        service.behavior.outputs.forEach((output) => {
          this.procedureContent += `<Output name="${output.name}" isOptional="${output.isOptional}" dataServiceId="${output.dataServiceId}"/>`;
        });
        this.procedureContent += `</Outputs></Behavior></TaskInstance>`;
      });
      this.procedureContent += ` </Configuration>`;
