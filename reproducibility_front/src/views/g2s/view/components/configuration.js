...
export function generateAction(
  ...
  action.modelItemList = generateModelItemList(
    models,
    allInputList,
    modelOutputInGraph,
    linkEdgeList
  ...
  if (type == "taskInstance") {
    action.dataItemList = [
      ...generateInputDataItemList(modelInputInGraph),
      ...generateOutputDataItemList(modelOutputInGraph),
    ];
  }
...
function generateModelItemList(
  models,
  allInputList,
  modelOutputInGraph,
  linkEdgeList
  ...
  models.forEach((model) => {
    ...
    let outputList = modelOutputInGraph.filter(
      (event) => event.md5 == model.md5
    );
...
function generateInputDataItemList(modelInputInGraph) {
  let inputList = modelInputInGraph.filter(
    (event) =>
...
export function generateXml(
  ...
  modelListInGraph.forEach((model) => {
    ...
    let inputList = list.filter(
      (event) => event.nodeAttribute.md5 == model.nodeAttribute.md5
    );
    let outputList = modelOutputInGraph.filter(
      (event) => event.nodeAttribute.md5 == model.nodeAttribute.md5
    );
    ...
    inputList.forEach((item) => {
      ...
      if (item.type == "modelServiceInput") {
        ...
        let link = linkEdgeList.filter(
          (el) => el.target.nodeAttribute.eventId == item.nodeAttribute.eventId
        );
  ...
  dataServiceListInGraph.forEach((service) => {
    ...
    let inputList = list.filter(
      (event) =>
        event.nodeAttribute.dataServiceId == service.nodeAttribute.dataServiceId
    );
    let outputList = dataServiceOutputListInGraph.filter(
      (event) =>
        event.nodeAttribute.dataServiceId == service.nodeAttribute.dataServiceId
    );
  ...
  dataServiceListInGraph.forEach((service, index) => {
    ...
    outputList.forEach((item, n) => {
      ...
      if (!item.nodeAttribute.upload) {
        DataProcessings.DataProcessing[index].Outputs.DataConfiguration[
          n
        ].Data = {
          _type: "insituData",
        };
...
export function getCellStyle(styleIn, cell) {
  ...
  if (
    cell.optional == "false" &&
    (cell.type == "input" || cell.type == "link")
  ) {
