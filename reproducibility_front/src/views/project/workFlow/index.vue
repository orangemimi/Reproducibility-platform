<template>
    <div class="mainContent">
      <div style="position:relative">
        <h1 class="title" :title="this.scenario.name" v-if="this.scenario.name"> {{ this.scenario.name }} </h1>
              ...
              <el-row class="rightContentCard" >
                <el-card>
                  <el-tabs v-model="activeExpected" class="instanceCard">
                    <el-tab-pane label="Service Instances" name="instance" style="display: flex; flex-wrap: wrap;justify-content:space-around">
                      <el-col
                        :span="7"
                        :offset="0"
  ...
  export default {
    ...
    computed: {
      instanceList(){
        ...
        return this.ExpectedInstances.map(item =>{
          ...
          let {id,index,isReproduced,updateTime,status,modelId,modelDescription} = item;
          let stateEnum = (status === "2") ? "success" : (status === "-1") ? "failed" : "running";
          let service = {};
    ...
    methods: {
      ...
      async getExpectedInstances(){
        ...
        let boundInstanceList = await getInstancesByInstances(this.scenario.instances);
