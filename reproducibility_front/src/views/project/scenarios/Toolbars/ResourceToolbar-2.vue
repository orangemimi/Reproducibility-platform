    ...
    <vue-scroll style="height: 905px; width: 100%" :ops="ops">
      ...
      >
        <el-tab-pane label="Models" name="models"
          ...
          ></model-item-toolbar
        ></el-tab-pane>
        <!-- <el-tab-pane label="Data Service Methods" name="dataServices">
...
<script>
import bus from "./bus";
import modelItemToolbar from "./ModelItemToolbar";
    ...
    selectItemListToGraph(val) {
      ...
      val.forEach((item, index) => {
        ...
        if (item.type == "input") {
          ...
          this.getSelectItemStyleAddToGraph(
            panel,
            item,
            this.currentCell.geometry.x - 100 + 200 * index,
            this.currentCell.geometry.y - 120
          );
        }
        if (item.type == "output") {
        ...
        if (
          hasProperty(item, "nodeType") &&
          (item.nodeType == "input" || item.nodeType == "parameter")
        ) {
    ...
    addCell(item, x, y, type, styleObj) {
      ...
      try {
        ...
        if (type == "dataServiceInput" || type == "dataServiceOutput") {
          ...
          vertex.nodeAttribute.isParameter =
            item.nodeType == "parameter" ? "true" : "false";
          // vertex.nodeAttribute.dataServiceId = item.dataServiceId;
          // vertex.nodeAttribute.token = item.token;
          // vertex.nodeAttribute.type = item.type; //Processing
          // vertex.linkModelCellId = selectionCell.id; //放置输入输出node时 与其关联的model的nodeId
