  ...
  <div class="leftContainer">
    ...
    <div v-show="switchValue" class="card-contain">
      <vue-scroll :ops="ops" style="max-height: 390px;">
        ...
        <div
          v-for="(model, index) in publicModels"
          :key="index"
          ref="modelItemList"
          v-else
        >
          <div class="choose-model-contain">
            <model-card
              :modelFrom="model"
              @click.native="getModelInfo(model)"
            ></model-card>
          </div>
        </div>
      </vue-scroll>
    ...
    <div v-show="!switchValue" class="card-contain">
      <vue-scroll :ops="ops" style="height: 300px">
        <div
          v-for="(model, index) in personalModels"
          :key="index"
          ref="modelItemList"
        >
          <div class="choose-model-contain">
...
import {
  getModelItemsByPrivacy,
  getModelItemsByProvider,
...
export default {
  ...
  methods: {
    async getPublicModels() {
      let { content } = await getModelItemsByPrivacy(
        "public",
    ...
    async getAvailableModels() {
      console.log(this.publicModelFilter);
      let { content } = await getAvailableModels(
        this.publicModelFilter.page,
        this.publicModelFilter.pageSize
      );
      // console.log(data);
    ...
    async getPersonalModels() {
      let data = await getModelItemsByProvider(
