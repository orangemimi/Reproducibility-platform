<template>
  <div>
    ...
    >
      ...
      <el-button class="config-btn" type="text" @click="view(item.oid)"
        >View</el-button
      >
      <el-button
        type="text primary"
    ...
    <el-dialog v-model="dialogVisible" width="30%">
      <div v-if="item.invokeServices != undefined">
        <el-row :gutter="20">
          <el-col
            :span="6"
            v-for="(service, index) in item.invokeServices"
            :key="index"
          >
            <div
      ...
      <div v-else>
        <el-empty
          description="There is no computing service. Click here to download the service to run locally!"
        ></el-empty>
      </div>
    </el-dialog>
  </div>
  ...
  methods: {
    ...
    init() {
      if (localStorage.selectDataServices != undefined) {
        ...
        if (
          temp[this.item.oid] == undefined ||
          temp[this.item.oid].length == 0
        ) {
          this.select = false
          this.badgeNum = 0
        } else {
    ...
    view(oid) {
      window.open(
        'http://geomodeling.njnu.edu.cn/dataApplication/' + oid,
        '_blank'
      )
    },
    ...
    confirm() {
      if (localStorage.selectDataServices != undefined) {
        ...
        let dataServiceItem = JSON.parse(
          localStorage.selectDataServices
        ).dataServiceItem
        if (this.badgeNum != 0) {
        ...
        if (
          dataServiceItem[this.item.oid] != undefined &&
          tempArr.length == 0
        ) {
        ...
        this.$store.state.user.selectNum =
          this.$store.state.user.selectNum + tempArr.length - this.badgeNum
